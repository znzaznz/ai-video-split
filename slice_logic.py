...
SLICE_LOGIC_MODES: dict[str, dict[str, str]] = {
    ...
    "按时间范围": {
        "summary": "适合：已知大概起止时间，直接裁出该区间；可用自然语言（如七分钟到八分钟）或 7:00-8:00。",
        "how": (
            "切片逻辑：按用户给出的起止时间精确裁剪。"
            "仅当输入中写明「前后各多留 X 秒」等时才向外扩展。"
        ),
        "coarse_how": "粗分：在用户给定时间范围内对齐句边界；未要求扩边则不额外延长。",
        "retrieval_hints": "优先解析目标中的起止时间，少做全文检索。",
        "default_text": "00:07:20–00:08:20（约 7 分 20 秒到 8 分 20 秒）。",
    },
    "高光混剪": {
